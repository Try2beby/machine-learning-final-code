...
def masked_accuracy(y_pred: torch.Tensor, y_true: torch.Tensor, mask: torch.Tensor):
    ...
    acc = (predicted == y_true).double.mean()
...
class Recommender(pl.LightningModule):
    def __init__(
        ...
        self.item_embedding = nn.Embedding(vocab_size, embedding_dim=channels)
        ...
        self.encoder = torch.nn.TransformerEncoder(encode_layer, num_layers=6)

        self.linear_out = Linear(channels, vocab_size)

        self.do = nn.Dropout(self.dropout)

    def encode_src(self, src_items):
        src_items = self.item_embedding(src_items)
    ...
    def training_step(self, batch, batch_idx):
        ...
        loss = masked_ce(y_pred, y_true, mask)
        accuracy = masked_accuracy(y_pred, y_true, mask)
